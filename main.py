...
Description: This script provides functionality to generate Tamil text-to-speech
             ...
             an MP3 file, and listen to the generated audio. Includes a social preview image display.
...
class ThirukkuralApp(QMainWindow):
    ...
    def create_sidebar(self):
        ...
        btn_social_preview = QPushButton("Social Preview")
        btn_social_preview.clicked.connect(lambda: self.switch_view("SocialPreview"))
        ...
        sidebar_layout.addWidget(btn_social_preview)
    ...
    def create_central_content(self):
        ...
        self.social_preview_widget = self.create_social_preview_widget()
        self.stacked_widget.addWidget(self.social_preview_widget)
    ...
    def create_social_preview_widget(self):
        """Creates a widget to display the social preview image."""
        widget = QWidget()
        layout = QVBoxLayout()
        ...
        image_label = QLabel()
        try:
            pixmap = QPixmap("social_preview.png")
            if not pixmap.isNull():
                # Scale the image to fit within 800x400px while maintaining aspect ratio
                pixmap = pixmap.scaled(800, 400, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                image_label.setPixmap(pixmap)
                image_label.setAlignment(Qt.AlignCenter)
            else:
                image_label.setText("Social preview image not found. Please place 'social_preview.png' (1280x640px) in the project directory.")
        except Exception as e:
            image_label.setText(f"Error loading social preview image: {e}")
    ...
    def switch_view(self, view_name):
        ...
        elif view_name == "SocialPreview":
            self.stacked_widget.setCurrentWidget(self.social_preview_widget)
        elif view_name == "Help":
